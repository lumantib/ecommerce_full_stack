...
import SellerPedingVerifiedProductFrom from './SellerPedingVerifiedProductFrom';
...
const SellerPendingVerifiedProductListPage = () => {
    useEffect(() => {
        fetchData()
    }, []);
    ...
    const fetchData = () => {
        publicRequest.get('/products/seller')
            .then(res => {
                setData(res?.data?.filter(product => { return !product.isVerified }))
            })
            ...
            <MuiModal handleOpen={handleOpen} handleClose={handleClose} open={open} setOpen={setOpen} >
                {
                    modalMode === identifiers?.add?.modal_mode && (
                        <ModalContainer title={identifiers.add.title} handleClose={handleClose} identifiers={identifiers} modalMode={modalMode}>
                            <SellerPedingVerifiedProductFrom
                                {...modalProps}
                            />
                        </ModalContainer>
                    )
...
export default SellerPendingVerifiedProductListPage